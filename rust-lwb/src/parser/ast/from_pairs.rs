...
impl<F, M: AstInfo> GenerateAstInfo for F where F: FnMut(&ParsePairSort) -> M {
    type Result = M;
...
pub trait FromPairs<M: AstInfo> {
    fn from_pairs<G: GenerateAstInfo<Result = M>>(pair: ParsePairSort, generator: G) -> Self where Self: Sized;
}
